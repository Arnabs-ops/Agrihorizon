...
export function FarmPortfolio({ userProfile, isReadOnly = false }: FarmPortfolioProps) {
    ...
    const handleSaveBio = async () => {
        try {
            ...
            toast.success(t('bioUpdated') || "Farm bio updated successfully!");
            setIsEditing(false);
        } catch (error) {
            toast.error(t('updateFailed') || "Failed to update bio");
        }
    };
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            toast.success(t('imageUploaded') || "Farm image uploaded!");
        } catch (error) {
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 md:p-12 border border-slate-100 dark:border-slate-800 modern-shadow relative overflow-hidden">
                ...
                <div className="relative z-10 space-y-8">
                    <div className="flex justify-between items-start">
                        <div>
                            <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-2">{t('digitalFarmPortfolio') || "Digital Farm Portfolio"}</h2>
                            <p className="text-slate-500 dark:text-slate-400 font-medium">{t('showcaseYourFarm') || "Showcase your farm to potential buyers"}</p>
                        </div>
                        {profile?.isVerified && (
                            <span className="bg-emerald-500/10 text-emerald-600 px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest border border-emerald-500/20 flex items-center gap-2">
                                <span className="animate-pulse">✓</span> {t('verifiedSellers') || "Verified Farm"}
                            </span>
                        )}
                    </div>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-3xl p-6 md:p-8 border border-slate-100 dark:border-slate-700">
                        ...
                        {isEditing ? (
                            <textarea
                                ...
                                placeholder={t('writeAboutFarm') || "Tell buyers about your farming practices, history, and values..."}
                            />
                        ) : (
                            <p className="text-slate-600 dark:text-slate-400 leading-relaxed font-medium whitespace-pre-wrap">
                                {bio || (t('noBioYet') || "No bio added yet. Tell your story to build trust with buyers!")}
                            </p>
                        )}
                    </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            {(!profile?.farmImages || profile.farmImages.length === 0) && (
                                <div className="col-span-full py-12 text-center bg-slate-50/50 dark:bg-slate-800/30 rounded-3xl border border-dashed border-slate-200 dark:border-slate-700">
                                    <p className="text-slate-400 dark:text-slate-500 font-bold">{t('noImagesYet') || "No photos uploaded yet"}</p>
                                </div>
                            )}
                        </div>
                    </div>
